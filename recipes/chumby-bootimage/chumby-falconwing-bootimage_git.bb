...
PRIORITY = "optional"
LICENSE = "GPLv2"
DEPENDS = "chumby-bootshell chumby-blobs chumby-stub elftosb2-native config-util-native virtual/kernel"
DEPENDS_${PN} = "chumby-bootshell chumby-blobs chumby-stub elftosb2-native config-util-native virtual/kernel"
PROVIDES = "virtual/bootloader virtual/chumby-bootimage"
RPROVIDES = "virtual/bootloader virtual/chumby-bootimage"
PR = "r8"

SRC_URI = "file://falconwing_chumby_sb.db"
...
do_install () {
    install -d ${D}/boot
    install -m 0755 ${WORKDIR}/falconwing_chumby_sb.db ${DEPLOY_DIR_IMAGE}/falconwing_chumby_sb.db
}

do_deploy () {

    install -d ${DEPLOY_DIR_IMAGE}
    elftosb2 -p ${DEPLOY_DIR_IMAGE} -c ${DEPLOY_DIR_IMAGE}/falconwing_chumby_sb.db -o ${DEPLOY_DIR_IMAGE}/bootstream-chumby.bin
